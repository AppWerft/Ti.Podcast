...
#define TAG "de.appwerft.podcast"

using namespace v8;

static Persistent<Object> bindingCache;

static void Tipodcast_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Tipodcast.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::TipodcastBindings::lookupGeneratedInit(
...
static void Tipodcast_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Tipodcast_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Tipodcast_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::TipodcastBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TipodcastBinding = {
	"de.appwerft.podcast",
	Tipodcast_init,
	Tipodcast_dispose
};
...
Java_de_appwerft_podcast_TipodcastBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("de.appwerft.podcast", &TipodcastBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::TipodcastBindings::lookupGeneratedInit));
}
